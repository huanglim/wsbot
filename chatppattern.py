...
'(你|和尚|小僧|真一|真一你)(是|叫)(谁|誰|什么|机器人|男|女)':['真一就是真一啊',
                             ...
                             '小僧...师傅说, 我应该大概是异于大家吧..',
                             '我是师傅的..弟子啊',],
'(你|和尚|小僧|真一|真一你)(又)?(修好|回来|学习|来了)':['小僧想念施主们, 师傅就又放我回来啦', '进修归来, 学业进步了!'],
...
'.*吃(的|了)?(肉|什么|的什么|狗)':['小僧只吃斋饭的, 阿弥陀佛','吃喜欢吃的呀'],
'.*吃(饭|午饭|晚饭)?了吗':['吃的可多了!不过师傅说不要贪图口腹之欲, 只让我吃那么一丁点..', '一般只能吃三五碗的..师兄弟们都比我吃的多!'],
...
'(官方|qq|QQ)群':['师傅告诉我一串神秘的代码,说是什么官方QQ群:2714406,2613407. 四群:623776142'],
...
'(再)?(讲|来|说|念|读)?(一|1|几)?(个|句|段)?(笑话|段子|xh)':[
    ...
    '研表究明，汉字的序顺是不响影阅读的。',
    '牛二带翠花去扬州逛街, 路遇卖房产. 跟牛二说到"大优惠,现在豪宅原价1000元宝,只需998,998豪宅立马带回家",牛二非常生气:"不要! 我是差那两元宝的人吗? 真的是! 我差的是998"',
    '“把你最值钱的东西交出来!”面对劫匪的恐吓，牛二二话不说，把翠花给推了出去。翠花一时间竟不知应该感动还是生气。',
    '牛二问翠花:“你对我有什么样的感觉呢?”“怀孕的感觉”“喔?什么是怀孕的感觉”“想吐又吐不出来的感觉啊，难受啊”。',
    '张三在街上逛, 忽然感到屁股上被谁摸了一下，回头一看，一个漂亮妹妹，对张三甜甜一笑，张三心里美滋滋的。这时只听这妹子说：牛二，你能不能改掉随处擦鼻屎的坏习惯。',
    '牛二忍受不了翠花的无理取闹，对翠花吼了一句，翠花马上大哭：“你竟然吼我?”翠花正准备大闹的时候。牛二“吼你怎么了?要不是看你长的国色天香，如花似玉，闭月羞花，倾国倾城，温柔善良，我特么早就打你了!”',
],
...
'(再|在)?(讲|吟|来|说|念|读|斗)?(一|1|几)?(首|个|句|段)?(诗|词|ys)':[
...
'.*宗师(进阶|要求)?$':['宗师可厉害了呢, 进阶条件要特殊内功，轻功，招架，拳脚，和任意一种特殊武器技能练到800级，最大内力100000,打通任督二脉.进阶后, 师门和追捕任务的潜能收益为总经验的千分之一加单次经验奖励!'],
...
'.*(任督二脉|开脉|冲脉|通脉)$':['宗师需要打通任督二脉，在练功房里点修炼按钮可以开始通脉，一般需要7W2内力，可以找逍遥的大佬给个BUFF，以及醉仙楼买瓶神仙醉降低要求,似乎和根骨也有关系. 成功后可获得10点先天属性点'],
...
'臂力$':['影响攻击，招架属性. 先天臂力1点=1点攻击，0.5点招架. 后天臂力1点=先天数值÷10的攻击, 先天数值÷10的招架'],
'根骨$':['影响防御，最大气血属性. 先天根骨1点=5气血（人物初始属性10气血）.后天根骨1点=先天数值的气血，先天数值÷10的防御'],
'身法$':['影响躲闪，出招速度，暴击属性. 先天身法1点=0.04攻速，0.1暴击，0.5命中，0.5闪避. 后天身法只加闪避'],
'悟性$':['提高你学习，读书，练习的速度，不节省潜能'],
'(悟性|学习)(装|套)':['衣服:武当;鞋子:武当;头:武当(或涟星的冰玉簪)加悟性,华山加学习;项链:杨不悔的项链;饰品:前期神龙令,中后花无缺的玉佩;护腕:崔莺莺的手镯或武当橙;腰带:峨眉(紫以上);'],
...
'当铺$':['在扬州城, 刷新时间为每天中午12点. 可以买到宝石,秘籍, 丹药等. 可花黄金刷新, 600金之后需用元宝刷新'],
...
'.*(襄阳|襄阳守卫)(怎么开|怎么玩|如何开|怎么进)':['襄阳城随机刷新密探，当你找到并击杀后会获得密函，在没有战事时交给郭靖触发襄阳守城任务(每次最多可以40人报名.萌新不要报名,直接等胜利后领奖励)，守城奖励每周只可领取一次，每周一重置'],
'蒙古密函':['襄阳城随机刷新密探，当你找到并击杀后会获得密函，在没有战事时交给郭靖触发襄阳守城任务(每次最多可以40人报名.萌新不要报名,直接等胜利后领奖励)，守城奖励每周只可领取一次，每周一重置'],
'.*(襄阳|襄阳守卫战|襄阳守卫|军功)$': ['襄阳守卫战每周一次，最多可以获得500+200点军功，可以兑换潜能、装备、功法。萌新不要报名,可以等大家通关后领取低保200点(领取后本周就不能打了)',],
'低保$':['低保指的是襄城打赢后的200军功，当他提示胜利后你去襄阳找老郭领，萌新不要报名,可以等大家通关后领取低保200点'],
'.*提升境界$':['施主达到了相应的要求, 就可以到扬州的中间找金古易就可以提升了'],
'.*合成$':['绿色残页10合成一本，蓝色30，黄色50，紫色100，橙色200，红色500'],
'结婚$':['可以和异性玩家结婚，举办婚礼需男方准备1314金. 其他玩家赠送礼金可以可以观礼，从宴席礼桌获得稀有食物，食物等级和宴席等级相关，完成后双方将平分礼金, 举办婚礼酒店2小时CD'],
'离婚(CD)?$':['在衙门可以办理离婚, 阿弥陀佛. 离婚冷却时间为7天'],
'玫瑰花$':['结婚后每天丈夫可以在关系里送妻子最多99多玫瑰花. 可直接卖商店. 一朵一金.'],

'召唤令':['召唤令, 黄色品质, boss出品. 随机召唤世界boss '],
'(进阶武学|进阶|技能进阶|武功进阶)':['武功1000级并且在武道塔里面打出足够的残页可以进阶, 武道塔30-60层, 每1,5,8层掉落进阶残页.技能进阶需要潜能，计算式为:20W*技能等级+30W(黄色3，紫色4，橙色5)'],
'(门派)?残页兑换':['残页兑换是对那些判师的人有用，方法为当前颜色残页1:1随机兑换当前颜色或者当前颜色以下的，若是整本秘籍，则先拆分成残页，然后再进行兑换'],
...
'武道(残页|书)':['武道残页，在武道塔 （进阶完门派技能进阶后）61层，68层，71层，78层掉落。每十页武道残页可以合成一本 武道书'],
'武道':['武道进阶系统分为武器,内功,轻功,拳法,招架5大类.第一次进阶需100万潜能加一本武道书,以后每次进阶递增200W潜能消耗和一本武道书(不同大类武道书消耗分开计算).'],
'御气':['技能加属性:内力上限+165000'],
'内功进阶(选项)?':['御气、防御、气血、根骨、臂力、年龄、免伤、终伤'],
'招架进阶(选项)?':['招架、气血、御气、根骨、悟性'],
'武器进阶(选项)?':['进攻、防御、命中、招架、臂力、闪躲、暴击'],
'轻功进阶(选项)?':['闪避、防御、御气、命中、身法'],
'拳脚进阶(选项)?':['命中、攻击、御气、防御、招架、臂力'],
...
'(收徒|玩家 拜师|出师|师徒)':['宗师大佬可以收徒，徒弟向师父请安可以获得相应收益(每日1万经验潜能)，徒弟出师师父可以获得奖励(金箱子)，师徒组队完成副本可获得十倍经验,潜能收益,每周最多十次.'],
...
'培元丹':['师门奖励, 培元丹在宗师之前, 不要卖, 直接吃, 不然很难直接10W内力.'],
...
'(唐诗剑法|躺尸)':['唐诗剑法, 武道塔底层出, 后期关卡技能有妙用. 装备基本剑法时： 躲闪：+1005 绝招 躺尸 出招时间：8秒 冷却时间：20秒 躺下装死，敌人将放弃对你的进攻'],
...
#橙色
'月饼':['中秋活动物品, 金古易处获取. 保质期很短, 施主使用之后可增加100点精力. 另外一块给随从, 可大幅增加采药, 钓鱼的工作效率, 持续一天.'],
...
'挖矿代码':['wa'],
#
'黄君子':['可以由华山或者武馆后勤哪里用门派贡献换,也可在门派战或者帮派战的时打华山的黄弟子或紫弟子出',],
'门派黄.*(命中)?':['打门派黄命中7000左右保证正常命中. 其中峨眉, 少林命中需求稍低, 6000-6500可勉强命中 '],

# 武当

# '武当.*速宗':['可以尝试先太极神功，绝门，太祖长拳800速宗.'],
...
# 少林
'(少林|少林派|少林寺)$':['虽然我并非少林, 但是我还是很喜欢少林. 里面有很多得道高僧! 特点：只收男弟子，需要剃度，防御高，气血长'],
...
# 丐帮
'丐帮$':['墨门行者，人数众多，势力极广，号称「天下第一大帮」，由来已不得考证，帮众以裘褐为衣,以跂蹻为服,日夜不休,以自苦为极。特点：以拳脚棍法为主，攻击效果突出'],
...
# code
'.*代码':['在设置->高级->打开自动出招, 在方框内会自动出现招式代码. 招式代码与你当前动作条里的技能顺序一致, 如需改动出招顺序, 直接调整顺序即可.'],
...
'qnjs(命令|解释|说明)':['命令格式为: qnjs 起始等级 到达等级 技能颜色. 真一可以接受最后的颜色为数字. 1是白,2是绿,3是蓝,4是黄,5是紫,6是橙'],
...
'.*双修':['双修是什么? 小僧不太明白..'],
'帮派':['在扬州的帮派驻地找帮派管理员可以创建或加入帮派. 帮派可以通过帮贡召唤帮派boss, 打帮派战获取装备等物资!创建帮派需1000金.'],
'(退帮|脱离帮派)(CD)?$':['退帮CD为1天.'],
...
'(四十二章经|42章经)':['查看鳌拜府攻略获取42章经, 可在后续关外大清龙脉获取奖励. '],
#天地会
'拂尘':['绿色, 师门物品, 天地会神尼可出'],
...
'(副本)?关外(\s|，|,|的)?(攻略|打法|怎么打|怎么过)?':['跳瀑布需300+轻功, 如先天臂力15开石门则需70臂力, 先天臂力高, 可略低. 攻略参考设置->帮助->副本攻略. 询问真一 副本名 掉落或者数据 可查询相应信息(后续副本更新中)'],
...
'(副本)?温府(\s|，|,|的)?(通关数据|数据)$':['数据为通关的参考值.武当:招架3300、血6万。华山:攻击命中3800+,(九剑100+或狂风400+)、+天师符。逍遥:北冥500+ 天师符。其他:攻3000+命中2550血4-6万躲闪900+'],
...
'(副本)?青城山(\s|，|,|的)?(通关数据|数据)$':['预估数据为:命中7400有几率打不中开了踏歌行的余沧海.但基本通过不必那么高命中,预计有6000+可试. 攻击6500、气血10万'],
...
'(副本)?衡山(\s|，|,|的)?(通关数据|数据)$':['预估数据为:命中7000基本够, 因为武当攻击较低, 会比较困难, 可先击杀刘府大院互掐的NPC后再去击杀费斌降低一点难度'],
...
'(副本)?嵩山(\s|，|,|的)?(通关数据|数据)$':['预估数据为:攻击8000-10000、命中8500-10000. 命中8800可大概率命中左冷禅. 华山攻击9500比较稳妥'],
...
'(副本)?桃花岛(\s|，|,|的)?(通关数据|数据)$':['预估数据为:华山（攻击、命中10500，困难桃花岛要攻击、命中13000）'],
...
'(桃花岛)?(桃花阵|九宫格|迷阵)':['原理是由1走到9，5(中间的方格)可以重复（其他数字不能重复），1只会分布在九宫格四个边的中间，1的对数必是9，桃花阵四个角落都是偶数，出口从9的方向一直走就能走出去。',],
#白驼山
'(副本)?白驼山(\s|，|,|的)?掉落$':['灵蛇杖法必刷(蛇岛武道进阶比较好，但蛇岛进阶是后期的事了..这阶段大概率缺命中，而且是紫功法成型快)。蛤蟆功高爆发打追捕挺好用，蟾蜍步法开花的时候能用一下',],
'(副本)?白驼山(\s|，|,|的)?(通关数据|数据)$':['预估数据为:华山攻击12000, 命中11000'],
'(副本)?白驼山(\s|，|,|的)?(攻略|打法|怎么打|怎么过)?':[''],
# #星宿海
'(副本)?星宿海(\s|，|,|的)?掉落$':['碧磷针必刷，能用到古墓。鼎的话，可以考虑刷一个炼药用(意义不大)，非洲人也可以2鼎换1针或刷400次兑换，飞星术似乎就华山打擂台能用到'],
'(副本)?星宿海(\s|，|,|的)?(通关数据|数据)$':['预估数据为:华山攻击13500 命中12000'],
'(副本)?星宿海(\s|，|,|的)?(攻略|打法|怎么打|怎么过)?':['此关只需击杀星宿老怪丁春秋和二师兄狮吼子就行了，其他人不会拦你,星宿老仙可先和他比试, 骗掉飞星. 忙乱技能建议至少两个. 进入副本前刷满蛇岛buff'],
# #冰火岛
'(副本)?冰火岛(\s|，|,|的)?掉落$':['玲珑火芝和离火珠至少要50万气血以上搜索,屠龙刀需打困难'],
'(副本)?冰火岛(\s|，|,|的)?(通关数据|数据)$':['预估数据为:华山（普通攻击14000，命中13000；困难：攻击27000，命中24000）'],
'(副本)?冰火岛(\s|，|,|的)?(攻略|打法|怎么打|怎么过)?':['把两条路的动物们灭干净了, 在石洞杀了谢逊, 完成度100%. 最西边是火山, 注意火毒伤害（玲珑火芝和离火珠至少要50万气血以上搜索).屠龙刀需打困难'],
# #移花宫
'(副本)?移花宫(\s|，|,|的)?掉落$':['涟星的冰玉簪顶级悟性装, 无缝控制利器, 刷之. 花无缺的玉佩顶级悟性装, 刷之. 明玉功、碧血照丹青(困难模式), 看自身情况吧'],
'(副本)?移花宫(\s|，|,|的)?(通关数据|数据)$':['预估数据为:华山（普通：攻击19000，命中19000；困难：攻击32000，命中30000）'],
'(副本)?移花宫(\s|，|,|的)?(攻略|打法|怎么打|怎么过)?':['汉子进去主动叫杀，妹子不会; 花径一直向南, 注意血量. 宫女会掉火折子，开启暗道可用.记下涟星和邀月房间花丛的数量,在卧室按照花的左右和数量移动床,进入密道,用火折子可再进下一个.点小匣子拿东西.'],
# #燕子坞
# '(副本)?燕子坞(\s|，|,|的)?掉落$':[''],
